...
def read_audio(audio_file):
    ...
    # # output = "./" + filename[0] + "_1." + filename[1] # app
    # output = "." + filename[1] + "_1." + filename[2] # test
...
def find_bin_max(boxes, maxes_per_box):
    ...
    for key in boxes.keys():
        ...
        for i in range(len(boxes[key])):
            # print "i: ", i
            # print "len key: ", len(boxes[key])
            # print "max intensities", max_intensities
            # print 'currently evaluated intensity', boxes[key][i][0]
            # print "min freq of max intensities", min(max_intensities)[0]

            if boxes[key][i][0] > min(max_intensities)[0]:
                # print "len of max intensities list: ", len(max_intensities)
                # print "num maxes allowed: ", maxes_per_box
                ...
                if len(max_intensities) < maxes_per_box:  # add if < number of points per box
                    ...
                    # print "replacing ", min(max_intensities)      
...
wavfile1 = extract_audio(directory, audio1)
raw_audio1 = read_audio(wavfile1)
print raw_audio1
bins_dict1 = make_horiz_bins(raw_audio1[:44100*120], 1024, 0, 512) #bins, overlap, box height
boxes1 = make_vert_bins(bins_dict1, 43)  # box width
ft_dict1 = find_bin_max(boxes1, 7)  # samples per box
print "created ft dict 1"

wavfile2 = extract_audio(directory, audio2)
raw_audio2 = read_audio(wavfile2)
bins_dict2 = make_horiz_bins(raw_audio2[:44100*30], 1024, 0, 512)
boxes2 = make_vert_bins(bins_dict2, 43)
ft_dict2 = find_bin_max(boxes2, 7)
print "created ft dict 1"

pairs = find_freq_pairs(ft_dict1, ft_dict2)
delay = find_delay(pairs)
print "delay: ", float(delay)/43
